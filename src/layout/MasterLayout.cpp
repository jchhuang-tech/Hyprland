...
void CHyprMasterLayout::calculateWorkspace(const int& ws) {
    ...
    if (WINDOWS == 1 && !centerMasterWindow) {
        ...
        float averageWidth = WSSIZE.x / MASTERS;
        float widthAccumulated = 0;
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID == PWORKSPACE->m_iID && nd.isMaster)
                widthAccumulated += averageWidth * nd.percSize;
        }
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID != PWORKSPACE->m_iID || !nd.isMaster)
                continue;
            ...
            nd.percSize *= WSSIZE.x / widthAccumulated;
            float WIDTH = averageWidth * nd.percSize;
        ...
        float averageHeight = WSSIZE.y / MASTERS;
        float heightAccumulated = 0;
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID == PWORKSPACE->m_iID && nd.isMaster)
                heightAccumulated += averageHeight * nd.percSize;
        }
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID != PWORKSPACE->m_iID || !nd.isMaster)
                continue;

            nd.percSize *= WSSIZE.y / heightAccumulated;
            float HEIGHT = averageHeight* nd.percSize;
    ...
    if (orientation == ORIENTATION_TOP || orientation == ORIENTATION_BOTTOM) {
        ...
        float averageWidth = WSSIZE.x / STACKWINDOWS;
        float widthAccumulated = 0;
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID == PWORKSPACE->m_iID && !nd.isMaster)
                widthAccumulated += averageWidth * nd.percSize;
        ...
        float averageHeight = WSSIZE.y / STACKWINDOWS;
        float heightAccumulated = 0;
        for (auto& nd : m_lMasterNodesData) {
            if (nd.workspaceID == PWORKSPACE->m_iID && !nd.isMaster)
                heightAccumulated += averageHeight * nd.percSize;
        ...
        for (auto& nd : m_lMasterNodesData) {
            ...
            // wlr_log(WLR_INFO, "WSSIZE.y: %f", WSSIZE.y);
            // wlr_log(WLR_INFO, "newTotalHeight: %f", heightAccumulated);
        ...
        float averageHeightL = WSSIZE.y / slavesLeftL;
        float averageHeightR = WSSIZE.y / slavesLeftR;
...
void CHyprMasterLayout::resizeActiveWindow(const Vector2D& pixResize, eRectCorner corner, CWindow* pWindow) {
    ...
    // orientation is master position: left meaning master window is on the left
    eOrientation orientation = PWORKSPACEDATA->orientation;
    ...
    if (RESIZEDELTA != 0) {
        ...
        if (PNODE->isMaster && getMastersOnWorkspace(PNODE->workspaceID) > 1) {
            ...
            // float heightLeft = 0;
            // int slaveCount = 0;
            auto checkMastersLeft = [&heightLeft, &slavesLeft, PWORKSPACEDATA, &slaveCount](auto it) {
                if (!it.isMaster) {
                    return;
                }
                slaveCount++;
                if (!it.isMaster && PWORKSPACEDATA->orientation == 4 && slaveCount % 2 == 1)
                    return;
            ...
            // float roomForSizeIncrease = heightLeft - slavesLeft * minHeight;
            ...
            //
            // slaveCount = 0;
            // auto resizeMastersLeft = [roomForSizeIncrease, resizeDiff, minHeight, PWORKSPACEDATA, SIZE, &slaveCount](auto &it) {
            //     if (!it.isMaster)
            //         return;
            //     slaveCount++;
            //     if (!it.isMaster && PWORKSPACEDATA->orientation == 4 && slaveCount % 2 == 1)
            ...
            // float heightLeft = 0;
            // int slaveCount = 0;
            auto checkSlavesLeft = [&heightLeft, &slavesLeft, PWORKSPACEDATA, &slaveCount](auto it) {
                if (it.isMaster) {
                    return;
                }
                slaveCount++;
            ...
            if ((PWORKSPACEDATA->orientation % 2 == 0 && TOP) || (PWORKSPACEDATA->orientation % 2 == 1 && LEFT)) {
                ...
                std::for_each(std::next(NODEIT), m_lMasterNodesData.end(), checkSlavesLeft);
                resizeDiff = RESIZEDELTA;
            }
            // float nodeSize = PWORKSPACEDATA->orientation % 2 == 1 ? PNODE->size.x : PNODE->size.y;
            // float roomForSizeIncrease = heightLeft - slavesLeft * minHeight;
            ...
            //
            // slaveCount = 0;
            // auto resizeSlavesLeft = [roomForSizeIncrease, resizeDiff, minHeight, PWORKSPACEDATA, SIZE, &slaveCount](auto &it) {
            //     if (it.isMaster)
            //         return;
            //     slaveCount++;
            //     if (!it.isMaster && PWORKSPACEDATA->orientation == 4 && slaveCount % 2 == 1)
            ...
            // };
            // if ((PWORKSPACEDATA->orientation % 2 == 0 && TOP) || (PWORKSPACEDATA->orientation % 2 == 1 && LEFT)) {
            //     std::for_each(std::next(REVNODEIT), m_lMasterNodesData.rend(), resizeSlavesLeft);
            // } else {
            //     std::for_each(std::next(NODEIT), m_lMasterNodesData.end(), resizeSlavesLeft);
            // }
        }
        ...
        slaveCount = 0;
        auto resizeSlavesLeft = [roomForSizeIncrease, resizeDiff, minHeight, PWORKSPACEDATA, SIZE, &slaveCount, PNODE](auto &it) {
            if (it.isMaster != PNODE->isMaster)
                return;
            slaveCount++;
            if (!it.isMaster && PWORKSPACEDATA->orientation == 4 && slaveCount % 2 == 1)
